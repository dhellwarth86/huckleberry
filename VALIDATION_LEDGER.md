...
| `backend/core/job_storage.py` (D.2) | 336 | (post-commit; new file) | NEW file — Huckleberry-original. Three SQLite tables (`jobs`, `dispatch_results`, `trade_outputs`) sharing the existing `~/.tracepoint/cache.db` via `core.storage.DB_PATH`. Lifecycle API: `create_job` (computes `pdf_sha1`, writes UUID4 id), `get_job`, `list_jobs` (sortable by gc/location/trade_scope/bid_due_date/status/created_at + filter by gc/status/state), `update_job_status`, `mark_dispatch_complete`. Persistence: `persist_dispatch_result(job_id, ctx)` walks `ctx.pages`, `persist_trade_outputs(job_id, ctx)` walks `ctx.trade_module_outputs` and JSON-serializes per-page per-trade `TradeModuleOutput` (with dataclass→dict conversion). Loading: `load_dispatch_results`, `load_trade_outputs` reverse the persistence. Stdlib `sqlite3` only. Verified end-to-end: four jobs persisted across the long-run chain (Silverleaf, Bearss, Shoppes, Vine Street); all four round-tripped exactly (live aggregate == DB aggregate on every metric); per-page page_type / sheet_num / sheet_title / legends_count / has_legend / has_schedule / raw_tables_json all reconstruct losslessly. | D.2 ship 2026-04-29 | (D.2 commit 1 of 2 `38f849d`) |
| `backend/core/dispatch_gate.py` (D.2 extension) | +8 / -1 lines | n/a (already tracked from B + D.1 baselines) | Optional `job_id: str | None = None` parameter added to `run_dispatch`. After Stage 13 completes, an 8-line block lazy-imports `core.job_storage` and calls `persist_dispatch_result` + `persist_trade_outputs` + `mark_dispatch_complete` inside a try/except that appends to `ctx.dispatch_warnings` on failure. `job_id=None` path is unchanged from D.1 — the entire D.2 block is gated behind `if job_id is not None:`. Verified by D.2 soft gate (10/10 round-trip assertions) and D.2 three-bidset hard gate (21/21 criteria across Bearss + Shoppes + Vine Street). | D.2 ship 2026-04-29 | (D.2 commit 1 of 2 `38f849d`) |
...
| D.2 Silverleaf reference run (job persistence) | `backend/scripts/d2_silverleaf_reference.py` (tracked reusable harness) — `create_job` + `run_dispatch(storage="auto", job_id=...)` + `update_job_status("dispatched")` + reference report | `backend/D2_REFERENCE_silverleaf.md` (committed via D.2 commit 1) | 2026-04-29 |
| D.2 soft gate (Silverleaf persistence round-trip) | `backend/scripts/d2_persistence_soft_gate.py` (tracked reusable harness) — 10 round-trip assertions on the persisted Silverleaf job (`get_job`, dispatch_results page count + schedule_sheet count + raw_tables count, trade_outputs page count, roofing fields aggregate == 338, glazing items aggregate == 107) | `backend/D2_SOFT_GATE_silverleaf.md` (committed via D.2 commit 2) | 2026-04-29 |
| D.2 three-bidset hard gate (Bearss + Shoppes + Vine Street) | `backend/scripts/d2_three_bidset_hardgate.py` (tracked reusable harness) — for each bidset: `create_job` + `run_dispatch(storage="auto", job_id=...)` + 7-criterion evaluation (dispatch completes / 90% sweep baseline / error rate <25% / tables on schedule pages / persistence round-trip exact / vault SHA-1s / frontend SHA-1s) | `backend/D2_HARD_GATE_three_bidset.md` + `D2_HARD_GATE_bearss-ave.md` + `D2_HARD_GATE_shoppes-at-avalon.md` + `D2_HARD_GATE_vine-street.md` (committed via D.2 commit 2) | 2026-04-29 |
...
### D.2 long-run chain headline numbers (measured 2026-04-29)

**Silverleaf reference run (`d2_silverleaf_reference.py`):**
- Job ID `45d58c49-2e78-41d7-91c4-759a7a8de0de`. Dispatch 137.6s with `storage="auto"` + `job_id` set. `pdf_sha1 = 76dc89072dae77c1b476b60da85870f3d799cd31`. Module output 338 roofing fields / 20+81+6 glazing items — exact parity with calibration iter 2 + D.1 hard gate. `dispatch_complete=True`; `filters_completed=['filter_1','filter_2','filter_4','filter_3','filter_5','stage_13_trade_modules']`.

**Soft gate (`d2_persistence_soft_gate.py`) — 10/10 PASS:**
- `get_job` returns full row; name/gc/trade_scope exact; `dispatch_results` row count == 40 (every page persisted); 18 pages classified `schedule_sheet`; 18 pages have populated `raw_tables_json`; `trade_outputs` row groups == 40; roofing fields aggregate over all pages == 338; glazing+door+storefront items aggregate == 107 (20+81+6).

**Three-bidset hard gate (`d2_three_bidset_hardgate.py`) — 21/21 PASS across 3 bidsets:**
- Bearss Ave (`1efc6ea4-8a94-4b4c-9128-262fd1a0e2ae`): 91 pages, dispatch 471.0s, **769/177/31/24** roofing/glazing/door/storefront — **byte-exact with sweep baseline** (769/177/31/24 measured 2026-04-28). 0 module errors. 24/24 schedule_sheet pages with raw_tables. Persistence round-trip exact. 7/7 criteria PASS (90% threshold floor: 692/159/27/21).
- Shoppes at Avalon (`725c44b8-1493-4ab6-b4a8-abd8507ac8d8`): 97 pages, dispatch 721.8s, **833/43/22/22** — roofing +3 vs sweep 830 (run-to-run variance, +0.36%, well within 90% floor of 750); glazing/door/storefront byte-exact with sweep. 0 module errors. 13/13 schedule_sheet pages with raw_tables. Persistence round-trip exact. 7/7 criteria PASS.
- Vine Street (`fa4868da-dd5b-4679-893f-3a44d5047f12`): 138 pages, dispatch 1194.5s, **1201/100/12/23** — **byte-exact with sweep baseline** (1201/100/12/23 measured 2026-04-28). 0 module errors. 41/41 schedule_sheet pages with raw_tables. Persistence round-trip exact. 7/7 criteria PASS (90% threshold floor: 1080/90/11/21).

**Aggregate across the chain:**
- 4 jobs persisted in `~/.tracepoint/cache.db` (Silverleaf + Bearss + Shoppes + Vine Street).
- 0 module exceptions across 326 pages × 2 trades = **652 module calls** (0.00% per-page error rate, all three bidsets).
- 78 schedule pages across 3 bidsets had raw_tables persisted as JSON in `dispatch_results.raw_tables_json` and reconstructed losslessly on `load_dispatch_results`.
- Total dispatch time 2524.9s (~42 min). Module-output reproducibility: Bearss + Vine Street produced byte-exact aggregates compared to 2026-04-28 sweep baseline (different days, different sessions, harness-direct vs wired-dispatch — same numbers).
- **Sacred floors held throughout** — backend 216/19/0 verified pre-chain and post-chain; all 5 vault-ruled module SHA-1s and all 5 frontend HTML SHA-1s verified pre-chain and post-chain (zero changes).
