...
| F15 | Phase G.5b — render optimization (PyMuPDF + Pandas + tiling 200-300 DPI — original Phase G/F1 scope, now last in chain because page classification, cache, Filter 4 hoist, and annotation persistence precede). | _____ | _____ | _____ | _____ |
| F16 | Phase G.6 — Stages 6-9 geometry/scale/callout extraction wiring (deferred since D.1; auto-pin INSTANCES wait on this — extractor already wired in G.5a CP1 and starts producing rows immediately once Stages 6-9 land). | _____ | _____ | _____ | _____ |
