...
| F14 | Phase G.5 — render optimization (PyMuPDF + Pandas + tiling 200-300 DPI — original Phase G/F1 scope, now last in chain because page classification, cache, and Filter 4 hoist must precede). May be reordered behind annotation-persistence (baseline items 5+6) per Daniel's pick at next planning conversation. | _____ | _____ | _____ | _____ |
| F15 | 3-bidset hard gate + deferred E.2.2 visual hard gate (closes Phase F/G chain permanently) | _____ | _____ | _____ | _____ |
