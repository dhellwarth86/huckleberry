...
| F16 | Phase G.6 — Stages 6-9 geometry/scale/callout extraction wiring shipped (deferred since D.1, parked through D.2/E/G.0-G.5b). New `_run_stages_6_9(engine, doc, page_idx)` helper in `dispatch_gate.py` runs vector polygon clustering via `GeometryMatrix.cluster_and_union_polygons` (vector-direct path; OpenCV `detect_contours` rasterized fallback path stays in `geometry_matrix.py` for future scanned-PDF handling, with G.5b's `render_page_tiled` plumbing as the consumer-in-waiting). `_run_trade_modules` rewired: `_build_dispatch_only_input(...)` call replaced with `_run_stages_6_9(...) + build_trade_input(geometry_result, ...)`. The D.1 zeroed-geometry helper RETIRED (never called outside dispatch_gate.py). **Karpathy implementation choice surfaced from Read step:** plan called for OpenCV path, but vector-direct `cluster_and_union_polygons` is faster + more accurate for vector PDFs (all 15 test bidsets) and already integrates scale scoring against ARCH_SCALES. Used by calibration/sweep harnesses. The G.5a CP1 auto-pin extractor (idle for 1 day waiting for `RoofingModule.equipment_pins` to populate) is now active. | Developer | 2026-05-09 | soft gate 7/7 CP MET · sacred floor 278 → 282/19/0 (+4 G.6 tests) · vault re-locked at NEW SHA-1 for `dispatch_gate c7a98c42…` (was `71e409a2…` post-G.5b); 5 trade-knowledge files + `pdf_engine` HELD · pre-flight Taco Bell dispatch: 10 equipment_pins on 6 of 88 pages (was 0/0 pre-G.6); types: drains/scuppers/pipe_boots/hatches/exhaust_fans · live UI verification deferred to next session per Daniel directive | `backend/G_6_GATE_REPORT.md` · commit `e3d03e3` → canon |
...
**Session length:** Continuation of same session as G.5b ship → G.6 plan execution → Stages 6-9 wiring → pre-flight Taco Bell empirical → soft gate
**Branch state:** `phase2-v0.3-G6-stages-6-9-geometry-wiring` HEAD `<canon commit pending>`; total 1 code commit + canon when this lands; not yet pushed (awaiting Daniel's go on push).
**What shipped:** Phase G.6 — Stages 6-9 geometry/scale/callout extraction wiring. New `_run_stages_6_9(engine, doc, page_idx)` helper in `dispatch_gate.py` runs vector polygon clustering via `GeometryMatrix.cluster_and_union_polygons`. `_run_trade_modules` rewired: `_build_dispatch_only_input(...)` call replaced with `_run_stages_6_9(...) + build_trade_input(geometry_result, ...)`. The D.1 zeroed-geometry helper RETIRED. **Auto-pin INSTANCES finally flow:** the G.5a CP1 auto-pin extractor (idle for 1 day waiting for `RoofingModule.equipment_pins` to populate) is now active. Pre-flight Taco Bell dispatch produced 10 equipment_pins across 6 of 88 pages (was 0/0 pre-G.6); pin types: drains, scuppers, pipe_boots, hatches, exhaust_fans.
**Sacred floors at session end:** Backend **282/19/0** (was 278 at G.5b close, +4 net: G.6 unit + integration tests). Frontend **28/28** held. Vault re-locked at NEW SHA-1 for `dispatch_gate c7a98c42e8760168491630e94df90448d2a0294e` (was `71e409a27caf78525d663fa6ca11d0e7a0c9b7ca` post-G.5b). HELD: 4 trade-knowledge files at G.5a baseline; `debug_module 78f71d9030cde3b173389603f5f39bd6bedaac07`; `pdf_engine eb5b8372f6f0c52de5a81b452399035c2b720551` (G.5b baseline).
**Stops fired:** One Karpathy step 1 read surfaced an implementation choice the plan didn't anticipate: plan said Stage 6 = `GeometryMatrix.detect_contours` (OpenCV path), but `geometry_matrix.py` has a vector-direct path (`cluster_and_union_polygons`) that's faster + more accurate for vector PDFs (all 15 test bidsets are vector). Karpathy minimum-implementation = use the vector path. G.5b's `render_page_tiled` plumbing stays ready for the OpenCV scanned-PDF fallback when a scanned bidset surfaces. Choice documented in `backend/G_6_GATE_REPORT.md` §4.
**What's pending for next role:** G.6 ship paperwork done with this commit. Branch push awaits Daniel's go. After push: live UI verification of auto-pin INSTANCES on Taco Bell (deferred to next session per Daniel directive 2026-05-09: *"Skip live UI; soft-gate on the pre-flight result alone"*). Next phase work: 3-bidset closing hard gate (Bearss + Silverleaf + Vine Street + deferred E.2.2 visual) closes the F/G chain permanently.
**Open questions for Daniel:** None on G.6. Pending for next planning conversation: closing hard gate scope + bidset selection, or Phase C.3c follow-on (real glazing palette) first.

**Next-eligible work:** Closing hard gate (Phase F/G chain close) OR Phase C.3c follow-on (glazing palette). Phase 10 (algorithmic Filter 4 replacement) remains parked until post-hard-gate.

**Banked discipline lessons (2026-05-09 — G.6):**
1. **When two implementation paths exist, prefer the one already proven by adjacent code.** The plan called for OpenCV `detect_contours` for Stage 6; the read showed `cluster_and_union_polygons` (vector path) is what calibration/sweep harnesses already use successfully. Karpathy minimum-implementation = pick the proven path, not the plan's first guess. The plan's first guess was based on a default assumption (rendered raster) that didn't apply to vector PDFs.
2. **Pre-flight empirical on real bidset is a faster + cheaper soft gate than full live UI.** G.6 pre-flight: 1 backend dispatch on Taco Bell, count equipment_pins on roofing output. 5 minutes of bash; produces a definitive yes/no on "does the wiring work on real data." Live UI (start servers, drive browser, eyeball) is the next layer of verification but it's not the soft-gate signal — the empirical pin count is. Use pre-flight first; live UI for end-to-end UX confirmation.
3. **Wall-clock cost of Stages 6-9 is +21% on 88-page Taco Bell** (400s → 486.6s). Vector clustering + scale scoring per page is the new cost. Within Daniel's "<400s = no current pain" tolerance band (still under the spirit of the bound). Phase 10 (algorithmic Filter 4 replacement, parked) remains the bigger lever for future wall-clock work.
4. **Auto-pin INSTANCES on roof-plan pages depend on callouts being inside the polygon bbox + 30% margin.** Pre-flight saw pins on Taco Bell pages 0/12/21/23/47, NOT on the project-scope pages (18/19) that G.4 hard-gate identified as "TPO Membrane Roofing" sources. This is correct: callout extraction (Stage 9) operates per-page based on polygon detection; project-scope pages identify the SCOPE, not the equipment-callout density. Future work: cross-check whether pages 18/19 produced polygons but no callouts inside them (= callout extraction working correctly, just nothing to find), or polygons + callouts but margin too tight (= tune margin).
