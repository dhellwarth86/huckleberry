...
| E.2.1 | Frontend strip: execute strip plan, create `frontend/src/Huckleberry_AI_phase2.v1.0.0.html`, stub `extractScope`/`classifyPage`, remove pipeline tab, re-number tabs, inline `DEFAULT_WASTE_FACTOR`, rewrite `extractPlanSetFromPdf` to rendering-only | NEXT-eligible — executes the E.2.0 strip plan |
| E.2.2 | Frontend connect: wire `apiClient` to real endpoints, populate scope tab from API, populate pages tab from API, add API smoke tests | Available after E.2.1 |
