...
### Last-1 — Phase G.6 (Stages 6-9 geometry/scale/callout extraction wiring — auto-pin INSTANCES finally flow) — 2026-05-09

- **Branch:** `phase2-v0.3-G6-stages-6-9-geometry-wiring` HEAD `<canon commit pending>`; 1 code commit + canon when this lands; awaiting push approval
- **Shipped:** Stages 6-9 wired into dispatch (deferred D.1 → "D.2/E", parked through D.2/E/G.0-G.5b). New `_run_stages_6_9(engine, doc, page_idx)` helper in `dispatch_gate.py` runs **vector polygon clustering** via `GeometryMatrix.cluster_and_union_polygons` (vector-direct path; OpenCV `detect_contours` rasterized fallback path stays in `geometry_matrix.py` for future scanned-PDF handling, with G.5b's `render_page_tiled` plumbing as the consumer-in-waiting). `_run_trade_modules` rewired: `_build_dispatch_only_input(...)` call replaced with `_run_stages_6_9(...) + build_trade_input(geometry_result, ...)`. The D.1 zeroed-geometry helper RETIRED (never called outside dispatch_gate.py — calibration/sweep harnesses construct TradeModuleInput manually, unaffected). 4 new tests in `test_dispatch_geometry.py` (2 unit + 2 integration; all RED before implementation, all GREEN after). **The G.5a CP1 auto-pin extractor (idle for 1 day waiting for `RoofingModule.equipment_pins` to populate) is now active.**
- **Floor delta:** Backend 278 → 282/19/0 (+4 G.6 tests). Frontend 28/28 held (no frontend code changes — auto-pin INSTANCES use the existing G.5a CP3 visual rendering + annotation table reads + Viewer overlay rebuild). **Vault re-locked at NEW SHA-1 for `dispatch_gate c7a98c42e8760168491630e94df90448d2a0294e`** (was `71e409a27caf78525d663fa6ca11d0e7a0c9b7ca` post-G.5b). HELD: 4 trade-knowledge files at G.5a baseline (`roofing_module 9a6085d4…`, `roofing_vocabulary 863ffb01…`, `glazing_module 1b449e97…`, `glazing_vocabulary 008ed191…`); `debug_module 78f71d9030cde3b173389603f5f39bd6bedaac07`; `pdf_engine eb5b8372f6f0c52de5a81b452399035c2b720551` (G.5b baseline).
- **Receipts:** `backend/G_6_GATE_REPORT.md` · commit `e3d03e3` (CP1+CP2) → canon · soft gate 7/7 CP MET · pre-flight Taco Bell dispatch: 10 equipment_pins on 6 of 88 pages (was 0/0 pre-G.6); types: drains/scuppers/pipe_boots/hatches/exhaust_fans · live UI verification deferred to next session per Daniel directive 2026-05-09: *"Skip live UI; soft-gate on the pre-flight result alone"*
- **Learning:** Karpathy step 1 read of `geometry_matrix.py` surfaced an implementation choice the plan didn't anticipate: plan said Stage 6 = `GeometryMatrix.detect_contours` (OpenCV path, would consume G.5b's tile API), but `geometry_matrix.py` has a vector-direct path (`cluster_and_union_polygons` at line 276) that's faster + more accurate for vector PDFs (all 15 test bidsets) and already integrates scale scoring against ARCH_SCALES. Calibration/sweep harnesses already use this path. Karpathy minimum-implementation = use the vector path. The G.5b tile API stays as plumbing for the (future) OpenCV scanned-PDF fallback when a scanned bidset surfaces.
- **Lesson banked:** (a) When two implementation paths exist, prefer the one already proven by adjacent code — calibration/sweep harnesses had been using `cluster_and_union_polygons` since C.2 successfully; the plan's first guess was based on a default assumption (rendered raster) that didn't apply to vector PDFs. (b) Pre-flight empirical on real bidset is a faster + cheaper soft gate than full live UI — 1 backend dispatch + count equipment_pins gives definitive yes/no on "does the wiring work on real data" in 5 minutes; live UI verification is the next layer for end-to-end UX confirmation but isn't the soft-gate signal. (c) Wall-clock cost of Stages 6-9 = +21% on 88-page Taco Bell (400s → 486.6s). Within Daniel's "<400s = no current pain" tolerance; Phase 10 (algorithmic Filter 4 replacement, parked) remains the bigger lever. (d) Auto-pin INSTANCES depend on callouts being INSIDE polygon bbox + 30% margin; pages with project scope (e.g. Taco Bell 18/19 = TPO scope source) and pages with callouts (0/12/21/23/47) are different things — Stage 9 operates per-page based on polygon detection.

### Last-2 — Phase G.5b (PyMuPDF tile rendering API + surgical theater cut + pipeline-wide theater audit) — 2026-05-09
...
### Last-3 — Phase G.5a (annotation persistence + TradeModule Protocol vocabulary unfreeze) — 2026-05-08
...
### Next-1 — Closing hard gate (3-bidset + deferred E.2.2 visual + live UI verification of G.6 auto-pins)

After G.6 ships, the closing hard gate becomes next-eligible. Bundles three deferred verifications that were soft-gated through the F/G chain.

- **Depends on:** G.6 shipped (auto-pin INSTANCES wiring active)
- **Scope:**
  1. **3-bidset hard gate** (Bearss + Silverleaf + Vine Street + Taco Bell live) — re-run end-to-end against the full Phase F/G chain. Validates that all 15 prior phases produce reproducible output across multi-bidset workload. Catches single-bidset (Silverleaf/Taco Bell only) testing bias that may have masked bidset-specific quirks.
  2. **Deferred E.2.2 visual hard gate** that was rolled into E.2.2 but not exercised on multiple bidsets.
  3. **G.6 live UI verification** — Daniel-driven Taco Bell in browser: auto-pins (drains/scuppers/pipe_boots/hatches/exhaust_fans) appear as dotted-ring + 🤖 overlays on roof-plan pages 0/12/21/23/47 etc; takeoff rows populated with PAGE labels; manual pins coexist; DELETE works first-click. (Pre-flight confirmed equipment_pins flow on backend; live UI verifies the persistence + frontend rendering layer end-to-end.)
- **Floor target:** All previous floors held + hard gate report PASS
- **Gate:** **Hard gate** — completes the entire Phase F/G chain
- **Risk:** Single-bidset testing bias throughout the chain may have masked bidset-specific quirks. G.6 wall-clock overhead (+21% on 88-page Taco Bell) compounds across 3 bidsets — Vine Street (138 pp) likely worst case.
- **Run:** Developer session with Daniel manually verifying browser; General writes gate report

**Also queued (lower priority):** Phase C.3c follow-on — real glazing palette implementation replacing G.5a skeletal stubs. Independent of closing hard gate.

### Next-2 — TBD pending closing hard gate outcome

- **Depends on:** closing hard gate result
...
- G.6 shipped soft gate. Next is **closing hard gate** (3-bidset + deferred E.2.2 visual + G.6 live UI verification of auto-pins on Taco Bell). Phase C.3c follow-on (real glazing palette) and Phase 10 (algorithmic Filter 4 replacement, parked) remain queued at lower priority. Branch push approval pending.
...
- Vault rule: 5 trade modules (roofing_module, roofing_vocabulary, glazing_module, glazing_vocabulary, debug_module) — SHA-1 verified at session boundaries. `dispatch_gate.py` and `pdf_engine.py` are *integration-frozen* but allowed to change in dedicated phases via the unfreeze ceremony (G.1, G.2, G.3, G.5b, G.6 were such phases for `dispatch_gate.py`; G.3 + G.5b for `pdf_engine.py`; G.5a for the 4 trade-knowledge files). **Vault SHA-1 baseline (post-G.6 re-lock 2026-05-09):** `roofing_module 9a6085d4…`, `roofing_vocabulary 863ffb01…`, `glazing_module 1b449e97…`, `glazing_vocabulary 008ed191…`, `debug_module 78f71d90…`, `dispatch_gate c7a98c42…`, `pdf_engine eb5b8372…`. **CANONICAL HASH METHOD: `sha1sum` of raw file content on disk (CRLF preserved on Windows), NOT `git hash-object`.**
- Backend sacred floor: 282 passed / 19 skipped / 0 failed
...
- **Mandate 2026-05-09 (prefer the path adjacent code already proves):** When two implementation paths exist and one is already in use by adjacent code (calibration/sweep harnesses, neighboring helpers), prefer the proven path over the plan's first guess. G.6 plan said `GeometryMatrix.detect_contours` (OpenCV); read of `geometry_matrix.py` showed `cluster_and_union_polygons` (vector-direct) was what calibration/sweep harnesses had been using since C.2 — faster + more accurate for vector PDFs. Karpathy minimum-implementation = use the proven path; document the choice in the gate report. Plans encode default assumptions (e.g. "rendered raster") that may not survive contact with the codebase.
- **Mandate 2026-05-09 (pre-flight empirical beats live UI for soft gates):** A backend dispatch on a real bidset + count of the load-bearing artifact (e.g. equipment_pins) is a faster + cheaper soft-gate signal than full live UI verification. G.6 pre-flight: 1 backend dispatch on Taco Bell, count equipment_pins on roofing output. 5 minutes of bash; produces definitive yes/no on "does the wiring work on real data." Live UI is the next layer (end-to-end UX confirmation) but isn't the soft-gate signal. Use pre-flight first; defer live UI to next session if soft-gate result is unambiguous.
